...
const FAQBg = () => {
  ...
  return (
    <svg className="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg"
      ...
      {cols.map((x) => (
        <line key={`v-${x}`} x1={x} y1="0" x2={x} y2="1000" stroke="#0075CF" strokeWidth="0.4" opacity="0.10" />
      ))}
      {rows.map((y) => (
        <line key={`h-${y}`} x1="0" y1={y} x2="1200" y2={y} stroke="#0075CF" strokeWidth="0.4" opacity="0.10" />
      ))}
      ...
      {cols.map((x) =>
        rows.slice(0, 4).map((y) => (
          <rect key={`pad-${x}-${y}`} x={x - 3} y={y - 3} width="6" height="6" rx="1.5"
            fill="#0075CF" opacity="0.20" />
        ))
      )}
      {/* Top spike nodes */}
      {[75, 225, 375, 525, 675, 825, 975, 1125].map((x) => (
        <circle key={`spike-${x}`} cx={x} cy={80} r="3" fill="#0075CF" opacity="0.28" />
      ))}
      {/* Bottom orange nodes */}
      {[75, 225, 375, 525, 675, 825, 975, 1125].map((x) => (
        <circle key={`bot-${x}`} cx={x} cy={920} r="3" fill="#FD5A1A" opacity="0.26" />
      ))}
    </svg>
  );
};
...
const faqGroups = [
  {
    category: "Admissions", icon: GraduationCap, color: "bg-[#0075CF]",
    questions: [
  ...
  {
    category: "Training", icon: BookOpen, color: "bg-[#FD5A1A]",
    questions: [
  ...
  {
    category: "Placements", icon: Briefcase, color: "bg-[#0075CF]",
    questions: [
  ...
  {
    category: "Fees & Certification", icon: Award, color: "bg-[#FD5A1A]",
    questions: [
          ...
          {faqGroups.map((group, gi) => (
            <motion.div key={group.category} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45, delay: gi * 0.08 }}
              ...
              <div className={`flex items-center gap-3 px-6 py-4 ${group.color}`}>
                <group.icon className="w-5 h-5 text-[#FDFEFE]" />
                <h3 className="font-bold text-[#FDFEFE] text-base tracking-wide">{group.category}</h3>
              </div>
                  ...
                  return (
                    <div key={qi}>
                      <button onClick={() => toggle(id)} className="w-full flex items-center justify-between gap-4 px-6 py-4 text-left hover:bg-slate-50 transition-colors">
                        <span className={`font-semibold text-sm md:text-base leading-snug transition-colors ${isOpen ? "text-[#0075CF]" : "text-slate-800"}`}>{item.q}</span>
                        <ChevronDown className={`w-4 h-4 flex-shrink-0 transition-all duration-200 ${isOpen ? "rotate-180 text-[#0075CF]" : "text-slate-400"}`} />
                      </button>
                      {isOpen && (
